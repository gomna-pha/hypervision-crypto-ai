...
FULLY FIXED VERSION - Corrected ADASYN and Feature Engineering
...
def create_enhanced_features(df):
    ...
    # Create a copy and ensure we have the required columns
    features = pd.DataFrame(index=df.index)
    
    # Use Close price (handle both MultiIndex and single index)
    if isinstance(df.columns, pd.MultiIndex):
        close_price = df[('Close', 'BTC-USD')] if ('Close', 'BTC-USD') in df.columns else df['Close'].iloc[:, 0]
        open_price = df[('Open', 'BTC-USD')] if ('Open', 'BTC-USD') in df.columns else df['Open'].iloc[:, 0]
        high_price = df[('High', 'BTC-USD')] if ('High', 'BTC-USD') in df.columns else df['High'].iloc[:, 0]
        low_price = df[('Low', 'BTC-USD')] if ('Low', 'BTC-USD') in df.columns else df['Low'].iloc[:, 0]
        volume = df[('Volume', 'BTC-USD')] if ('Volume', 'BTC-USD') in df.columns else df['Volume'].iloc[:, 0]
    else:
    ...
    features['returns'] = close_price.pct_change()
    features['log_returns'] = np.log1p(close_price.pct_change())
    features['high_low_ratio'] = high_price / (low_price + 1e-10)
    features['close_open_ratio'] = close_price / (open_price + 1e-10)
    features['range'] = (high_price - low_price) / (close_price + 1e-10)
    
    # Moving averages
    for period in [5, 10, 20, 50]:
        features[f'sma_{period}'] = close_price.rolling(period).mean()
        features[f'ema_{period}'] = close_price.ewm(span=period, adjust=False).mean()
        features[f'volume_sma_{period}'] = volume.rolling(period).mean()
        features[f'price_to_sma_{period}'] = close_price / (features[f'sma_{period}'] + 1e-10)
    
    # Volatility features
    for period in [5, 10, 20]:
        features[f'volatility_{period}'] = features['returns'].rolling(period).std()
        features[f'volume_volatility_{period}'] = volume.pct_change().rolling(period).std()
    ...
    for period in [10, 20]:
        ...
        # Fixed: Calculate bb_position as a single value, not a DataFrame
        features[f'bb_position_{period}'] = (close_price - features[f'bb_lower_{period}']) / (features[f'bb_width_{period}'] + 1e-10)
    
    # Volume indicators
    features['volume_ratio'] = volume / (volume.rolling(20).mean() + 1e-10)
    features['vwap'] = (close_price * volume).cumsum() / (volume.cumsum() + 1e-10)
    features['vwap_ratio'] = close_price / (features['vwap'] + 1e-10)
    features['obv'] = (np.sign(close_price.diff()) * volume).cumsum()
    
    # Price patterns
    features['higher_high'] = (high_price > high_price.shift(1)).astype(int)
    features['lower_low'] = (low_price < low_price.shift(1)).astype(int)
    features['inside_bar'] = ((high_price < high_price.shift(1)) & 
                              (low_price > low_price.shift(1))).astype(int)
    
    # Market microstructure
    features['spread'] = high_price - low_price
    features['spread_pct'] = features['spread'] / (close_price + 1e-10)
    features['volume_price_trend'] = (volume * close_price.pct_change()).cumsum()
    ...
    # Rolling statistics
    for period in [5, 10]:
        features[f'return_skew_{period}'] = features['returns'].rolling(period).skew()
        features[f'return_kurt_{period}'] = features['returns'].rolling(period).kurt()
    
    # Remove NaN rows
    features = features.dropna()
    
    # Create labels (Hold=0, Buy=1, Sell=2)
    future_returns = close_price.shift(-1) / close_price - 1
    ...
    X = features.values[:-1]  # Remove last row (no future return)
    y = labels.values[:-1]
...
def backtest_strategy(df, predictions, initial_capital=10000):
    ...
    # Get close prices
    if isinstance(df.columns, pd.MultiIndex):
        prices = df[('Close', 'BTC-USD')].values if ('Close', 'BTC-USD') in df.columns else df['Close'].iloc[:, 0].values
    else:
...
def main():
    ...
    # Apply ADASYN balancing - FIXED: No n_jobs parameter
