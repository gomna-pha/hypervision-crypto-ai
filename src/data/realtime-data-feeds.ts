    ...
    try {
      ...
      ws.onmessage = (event) => {
        ...
        if (data.stream && data.data) {
          this.handleBinanceMessage(data.data);
        }
      };
 ...
 * await realtimeFeeds.connectBinance(['BTCUSDT', 'ETHUSDT']);
 * await realtimeFeeds.connectCoinbase(['BTC-USD', 'ETH-USD']);
 * await realtimeFeeds.connectKraken(['XBT/USD', 'ETH/USD']);
 * 
 * // Subscribe to price updates
 * realtimeFeeds.onPriceUpdate((feed) => {
 *   console.log(`${feed.exchange} ${feed.symbol}: $${feed.spotPrice}`);
 * });
 * 
 * // Calculate arbitrage spread
 * const spread = realtimeFeeds.calculateSpread('BTC');
 * if (spread.profitable) {
 *   console.log(`Arbitrage opportunity: ${spread.spread} bps`);
 * }
 */
